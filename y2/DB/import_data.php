...
<?php
    ...
    if (($csv = fopen('tutors.csv', 'r')) !== FALSE)
    {
        ...
        while($row = fgetcsv($csv, 1000))
        {
            ...
            else
            {
                ...
                $query = "INSERT IGNORE INTO user(emailAddress, password, userID, userType) VALUES ('$emailAddress', '$admin_pwd', $r, 'T');";
    ...
    if (($csv = fopen('students.csv', 'r')) !== FALSE)
    {
        $row = fgetcsv($csv, 1000);
    ...
    {
        if (($csv = fopen('tutors.csv', 'r')) === FALSE || ($csv = fopen('students.csv', 'r')) === FALSE) return;
        errorMsg($conn);
    }
    else
    {
