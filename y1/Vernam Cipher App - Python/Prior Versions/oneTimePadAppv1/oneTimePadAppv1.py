...
class oneTimePadApp(wx.Frame):
    ...
    def initUI(self):
        ...
        self.msgEntryLabel = wx.StaticText(self.panel1, label = '')
        self.labelBox1.Add(self.msgEntryLabel, flag = wx.TOP|wx.BOTTOM, border = 10)

        self.verticalBoxMain.Add(self.labelBox1, flag = wx.TOP|wx.BOTTOM|wx.LEFT, border = 10)
        #Text Box
    ...
    def encrypt(self, e):
        text = self.plaintxtInput.GetValue()
        pad = ''
        ciphertxt = one_time_pad(text, 'e', pad)[0]
        #^This is a different encryption from the one below!
        pad = one_time_pad(text, 'e', pad)[1]

        output = 'Ciphertext: {}  Pad Code: {}'.format(ciphertxt,pad)
    ...
    def decrypt(self, e):
        ...
        plaintxt = one_time_pad(text, 'd', pad)
        
        self.output.SetLabel('Plaintext: {}'.format(plaintxt))
        print (plaintxt)
