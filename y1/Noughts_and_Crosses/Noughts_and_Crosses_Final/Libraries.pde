...
boolean gameScreenVisible, optionSelected, multiplayerActive, gameScreenDisplayed = false;
...
void reset() {
  ...
  gameScreenVisible = optionSelected = multiplayerActive = gameScreenDisplayed = false;
...
boolean emptyCheck(gridTile tile) {
  if (tile.value.equals("null") == false) {
...
gridTile position() {
  ...
  for (int i = 0; i < grid.size(); i++) {
    if (grid.get(i).x == x1 && grid.get(i).y == y1)
    {
      tile = grid.get(i);
    }
  }
...
void winProcedure() {
  ...
  if (!won) {
    if (noughtWon) {
      ...
      won = true;
      gameScreenVisible = false;
    } else if ((turns == 11) && crossWon == false && noughtWon == false) {
